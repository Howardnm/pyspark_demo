...
# TODO: partition key 指定key分区发送数据

# 调用生产者的send方法即可发送数据
for i in range(10):
    futher = producer.send(
        topic='testpython',
        value=f"I love you".encode("UTF-8"), # encode("UTF-8") 等价于 b'I love you'
        key=b'my', # 指定key发送数据（相同key的数据会被分配到同一个分区，以key的hash值决定分区编号）
    )
    ...
    for key in [b'key1', b'key2', b'key3']:
        futher = producer.send(
            topic='testpython',
            value=f"Message for {key.decode()}".encode("UTF-8"),
            key=key,
        )
